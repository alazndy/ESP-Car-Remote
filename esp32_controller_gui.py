...
class DraggableJoystick:
    def __init__(self, canvas, c_x, c_y, b_r, k_r, d_r):
        self.canvas = canvas
        self.c_x, self.c_y = c_x, c_y
        self.b_r, self.k_r, self.d_r = b_r, k_r, d_r
        self.k_x, self.k_y = c_x, c_y
        self.is_dragging = False

        self.base = self.canvas.create_oval(c_x-b_r, c_y-b_r, c_x+b_r, c_y+b_r, fill="#2A2A2A", outline="#4A4A4A", width=4)
        self.knob = self.canvas.create_oval(c_x-k_r, c_y-k_r, c_x+k_r, c_y+k_r, fill=BUTTON_COLOR, outline="black", width=2)

        self.canvas.tag_bind(self.knob, "<Button-1>", self.on_press)
    ...
    def on_press(self, event):
        ...
        self.canvas.itemconfig(self.knob, fill=ACCENT_COLOR)

    def on_release(self, event):
        self.is_dragging = False
        self.canvas.itemconfig(self.knob, fill=BUTTON_COLOR)
        self.k_x, self.k_y = self.c_x, self.c_y
        self.canvas.coords(self.knob, self.c_x-self.k_r, self.c_y-self.k_r, self.c_x+self.k_r, self.c_y+self.k_r)

    def on_drag(self, event):
        if not self.is_dragging:
            return

        dx, dy = event.x - self.c_x, event.y - self.c_y
        dist = math.sqrt(dx**2 + dy**2)

        if dist > self.b_r:
            angle = math.atan2(dy, dx)
            self.k_x = self.c_x + self.b_r * math.cos(angle)
            self.k_y = self.c_y + self.b_r * math.sin(angle)
        else:
            self.k_x, self.k_y = event.x, event.y
    ...
    def get_direction(self):
        ...
        dx, dy = self.k_x - self.c_x, self.k_y - self.c_y
        dist = math.sqrt(dx**2 + dy**2)

        if dist < self.d_r: # Deadzone
            return None
        
        # Determine direction based on largest displacement
        if abs(dx) > abs(dy):
...
def send_command(command_char):
    if ser and ser.is_open and command_char:
        try:
            print(f"Gönderiliyor: '{command_char}'")
...
def create_styled_button(parent, text, command):
    button = tk.Button(parent, text=text, font=BUTTON_FONT, fg=TEXT_COLOR, bg=BUTTON_COLOR,
                       activebackground=ACCENT_ACTIVE_COLOR, activeforeground=TEXT_COLOR,
                       relief="flat", bd=0, command=command, width=15, height=2)
    button.bind("<Enter>", lambda e: button.config(bg=ACCENT_COLOR))
...
def create_main_window():
    ...
    # --- Pinout Şeması ---
    pinout_frame = tk.LabelFrame(left_column, text="Fiziksel Bağlantı Şeması", font=LABEL_FONT, padx=15, pady=15, bg=FRAME_COLOR, fg=TEXT_COLOR, bd=0)
    pinout_frame.grid(row=0, column=0, sticky="nsew")
    pinout_text = tk.Text(pinout_frame, bg=FRAME_COLOR, fg=TEXT_COLOR, font=PINOUT_FONT, height=15, width=40, bd=0, highlightthickness=0)
    pinout_text.pack(fill="both", expand=True)
    pinout_text.insert(tk.END, """
ESP32 Pinout:
Joystick X-Axis: GPIO 34 (ADC1_CH6)
Joystick Y-Axis: GPIO 35 (ADC1_CH7)
Joystick Switch: GPIO 32
Back Button:     GPIO 19
Play/Pause:      GPIO 18
Assistant:       GPIO 5
Volume Up:       GPIO 17
...
SD:  GPIO 22
""")
    pinout_text.config(state="disabled") # Sadece okunabilir yap
    ...
    # --- Sanal Kumanda ---
    joystick_frame = tk.LabelFrame(right_column, text="Sanal Kumanda", font=LABEL_FONT, padx=15, pady=15, bg=FRAME_COLOR, fg=TEXT_COLOR, bd=0)
    joystick_frame.grid(row=0, column=0, sticky="nsew", pady=(0,10))
    joystick_canvas = tk.Canvas(joystick_frame, bg=FRAME_COLOR, highlightthickness=0, width=250, height=250)
    joystick_canvas.pack(fill="both", expand=True)
    joystick = DraggableJoystick(canvas=joystick_canvas, c_x=125, c_y=125, b_r=80, k_r=30, d_r=15)
    joystick_canvas.bind("<Double-Button-1>", lambda e: send_command('t'))
    tk.Label(joystick_frame, text="Yönler için sürükle, onay için çift tıkla", font=("Segoe UI", 8), bg=FRAME_COLOR, fg="#AAAAAA").pack(pady=(5,0))

    # --- Sanal Butonlar ---
    other_buttons_frame = tk.LabelFrame(right_column, text="Sanal Butonlar", font=LABEL_FONT, padx=15, pady=15, bg=FRAME_COLOR, fg=TEXT_COLOR, bd=0)
    other_buttons_frame.grid(row=1, column=0, sticky="nsew")

    button_commands = {
        "Geri (b)": 'b', "Oynat/Duraklat (p)": 'p', "Asistan (x)": 'x',
        "Ses Yükselt (y)": 'y', "Ses Azalt (l)": 'l',
        "Sonraki (i)": 'i', "Önceki (g)": 'g'
    }
    
    button_row = 0
    button_col = 0
    for text, cmd in button_commands.items():
        button = create_styled_button(other_buttons_frame, text, lambda c=cmd: send_command(c))
        button.grid(row=button_row, column=button_col, padx=5, pady=5)
        button_col += 1
        if button_col > 1: # Her satırda 2 buton
            button_col = 0
            button_row += 1
...
def show_connection_window():
    def try_connect():
        nonlocal ser, p, stream, audio_thread_running
        port = selected_port.get()
        try:
            ser = serial.Serial(port, 115200, timeout=1)
            time.sleep(2) # ESP32'nin başlaması için bekle
    ...
    conn_win = tk.Tk()
    conn_win.title("Bağlantı Kur")
    conn_win.geometry("350x150")
    conn_win.config(bg=BG_COLOR)
    ...
    if not port_list:
        messagebox.showerror("Hata", "Hiç COM portu bulunamadı!")
        conn_win.destroy()
        return
    
    selected_port = tk.StringVar(conn_win)
    selected_port.set(port_list[0]) # İlk portu varsayılan olarak seç
    
    option_menu_style = {"bg": BUTTON_COLOR, "fg": TEXT_COLOR, "activebackground": ACCENT_COLOR, "relief": "flat", "font": BUTTON_FONT}
    dropdown = tk.OptionMenu(conn_win, selected_port, *port_list)
    dropdown.config(**option_menu_style)
    dropdown["menu"].config(**option_menu_style)
    dropdown.pack(pady=5, padx=20, fill="x")
    
    connect_button = create_styled_button(conn_win, "Bağlan", try_connect)
